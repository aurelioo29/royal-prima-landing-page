...
import type { TermsConditionsResolvedSection } from "@/components/terms-and-conditions";
...
export async function generateMetadata({
  params,
}: TermsConditionsPageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({
    ...
    namespace: "TermsConditionsPage.metadata",
  });
...
export default async function TermsConditionsPage({
  ...
  const t = await getTranslations("TermsConditionsPage");

  const sections: TermsConditionsResolvedSection[] =
    ...
    <>
      ...
      <Reveal direction="up" distance={24}>
        <TermsConditionsContent
          ...
          contactEmail={termsConditionsConfig.email}
          contactHref={termsConditionsConfig.emailHref}
        />
      </Reveal>
    </>
  );
