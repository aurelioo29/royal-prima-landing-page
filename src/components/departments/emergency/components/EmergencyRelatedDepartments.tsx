...
import type { EmergencyRelatedDepartmentsProps } from "../types/emergency.types";

import EmergencyIcon from "./EmergencyIcon";

export default function EmergencyRelatedDepartments({
  eyebrow,
  title,
  description,
  items,
  viewLabel,
}: EmergencyRelatedDepartmentsProps) {
  return (
        ...
        <div className="mt-10 grid grid-cols-1 border-t border-[#D7E7EE] md:grid-cols-2">
          {items.map((item) => (
            <Link
              ...
              href={item.href}
              className="group border-b border-[#D7E7EE] py-6 text-[#123B56]! no-underline! md:px-6 md:odd:border-r md:odd:pl-0"
            >
              <div className="flex items-end justify-between gap-6">
                ...
                <span className="flex size-10 shrink-0 items-center justify-center bg-white text-[#0077B6] transition-colors group-hover:bg-[#0077B6] group-hover:text-white">
                  <EmergencyIcon name="arrow" className="size-[17px]" />
                </span>
              </div>
