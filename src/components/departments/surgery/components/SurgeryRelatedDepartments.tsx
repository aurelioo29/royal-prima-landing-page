...
import type { SurgeryRelatedDepartmentsProps } from "../types/surgery.types";

import SurgeryIcon from "./SurgeryIcon";

export default function SurgeryRelatedDepartments({
  eyebrow,
  title,
  description,
  viewLabel,
  items,
}: SurgeryRelatedDepartmentsProps) {
  return (
            ...
            >
              <div className="flex items-end justify-between gap-6">
                ...
                <span className="flex size-10 shrink-0 items-center justify-center bg-white text-[#0077B6] transition-colors group-hover:bg-[#0077B6] group-hover:text-white">
                  <SurgeryIcon name="arrow" className="size-[17px]" />
                </span>
              </div>
