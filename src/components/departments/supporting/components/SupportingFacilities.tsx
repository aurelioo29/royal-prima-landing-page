import type { SupportingFacilitiesProps } from "../types/supporting.types";

import SupportingIcon from "./SupportingIcon";

export default function SupportingFacilities({
  eyebrow,
  title,
  description,
  items,
}: SupportingFacilitiesProps) {
  return (
        ...
        <div className="mt-12 border-t border-l border-[#D7E7EE] lg:mt-16">
          <div className="grid auto-rows-fr grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
            {items.map((item, index) => (
              <article
                key={item.key}
                className="group relative flex min-h-[265px] flex-col border-r border-b border-[#D7E7EE] p-7 transition-colors duration-300 hover:bg-white lg:p-8 xl:min-h-[285px] xl:p-9"
              >
                ...
                <span className="flex size-12 items-center justify-center bg-white text-[#0077B6] transition-colors duration-300 group-hover:bg-[#0077B6] group-hover:text-white">
                  <SupportingIcon name={item.icon} className="size-[21px]" />
                </span>

                <div className="mt-auto pt-10">
                  <h3 className="m-0! text-[20px] leading-7 font-bold text-[#123B56]">
                    {item.title}
                  </h3>
