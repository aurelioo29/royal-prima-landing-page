...
import type { InpatientRelatedDepartmentsProps } from "../types/inpatient.types";

import InpatientIcon from "./InpatientIcon";

export default function InpatientRelatedDepartments({
  eyebrow,
  title,
  description,
  viewLabel,
  items,
}: InpatientRelatedDepartmentsProps) {
  return (
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex items-end justify-between gap-6">
                <div>
                  <h3 className="m-0! text-[20px] font-bold transition-colors duration-300 group-hover:text-[#0077B6]">
                    {item.title}
                  </h3>
                ...
                <span className="flex size-10 shrink-0 items-center justify-center bg-white text-[#0077B6] transition-colors duration-300 group-hover:bg-[#0077B6] group-hover:text-white">
                  <InpatientIcon name="arrow" className="size-[17px]" />
                </span>
              </div>
